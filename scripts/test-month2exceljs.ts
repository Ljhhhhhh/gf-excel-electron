/**
 * 测试 month2exceljs 模板的生成脚本
 * 使用方法：npx tsx scripts/test-month2exceljs.ts
 */
...
async function main() {
  console.log('========== 测试 month2exceljs 模板 ==========\n')
  ...
  const reportResult = await dataToReport({
    ...
    outputDir,
    reportName: `month2-test-${Date.now()}.xlsx`,
    userInput: {
      queryYear: 2025,
      endMonth: 6
    }
  ...
  console.log('  - 7列（A列行业名 + B-G列1-6月数据）')
